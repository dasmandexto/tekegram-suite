...
- **broadcast** — рассылка по opt-in подписчикам: dry-run по умолчанию, opt-out,
  ...
  паузы, авто-заморозка аккаунта при флуд-лимите.
...
- **рассылка**: текст (можно `{username}`), кнопка «Предпросмотр (dry-run)»
  с числом и списком получателей, кнопка «Отправить» (после подтверждения);
...
`/api/broadcast/unsubscribe`, `/api/tasks/{id}` (статус фоновой задачи).
